...
if st.button(
    ...
    if not traj_df.empty:
        ...
        st.caption(
            "Raw values for one selected recording file. Each line is the median trajectory for a phonemic or coarticulatory nasality condition."
        )
        cue_labels = {
            "A1_P0_dB": "A1-P0 (dB)",
        ...
        st.caption(
            "Each file is z-scored using oral CVC tokens as reference; the plot then shows median curves across all files."
        )
        for cue in selected_core_cues:
            plot_data = core_trajectory_preview_data(
                traj_df,
